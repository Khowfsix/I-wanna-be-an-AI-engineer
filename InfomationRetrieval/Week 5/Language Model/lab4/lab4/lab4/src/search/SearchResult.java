...
/**
 * SearchResult lưu kết quả tìm được, gồm internal ID (docid), external ID
 * (docno), và relevance score
 */
public class SearchResult {
	 ...
	 *            Danh sách kết quả
	 * @param n
	 *            Chỉ lấy top-n kết quả
	 */
	public static void writeTRECFormat(PrintStream writer, String queryid, String runname, List<SearchResult> results,
			int n) {
		for (int ix = 0; ix < results.size() && ix < n; ix++) {
			int rank = ix + 1;
			writer.printf("%-2s 0 %-12s %-3d %.8f %16s\n", queryid, results.get(ix).docno, rank, results.get(ix).score, runname);
			rank++;
			if (rank > n) {
				break;
			}
